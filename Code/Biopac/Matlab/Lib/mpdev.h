/*! \file mpdev.h
	...
	The Hardware API is compatible with the MP150 communicating via Ethernet using UDP and the MP35/MP36 communicating via USB.
...
typedef enum MP_TYPE
{
	...
	MP36			/**< represents the MP36 device */
} MPTYPE;
...
typedef enum TRIGGER_OPT
{
	...
	MPTRIGDCH	/**< represents digital channel triggering (MP35/MP36 only)*/
} TRIGGEROPT;
...
/** Enumerated Values For MP35/MP36 Device Ouput modes
 *
 *  Represents supported MP35/MP36 output modes. 
 *
 */
typedef enum MP3XOUTPUT_OPT
{
	OUTPUTVOLTAGELEVEL		= 2, /**< = Setup ouptput mode to CONSTANT VOLTAGE LEVEL (MP35 device ONLY) */
	OUTPUTCHANNEL3			= 3, /**< = Redirect input channel #3 to output  */
//	OUTPUTSTIMULATOR		4,	// Not supported with BHAPI 2.0
 ...
 *
 *	\note
 *		- For the MP35/MP36, the Scaling and the Hardware Configuration elements are applied 
 ...
 *
 *	\note	
 *		- The MP35/MP36 has dedicated input and output lines while the MP150 has auto-switching digital input and output lines
 *		- For the MP150, this function automatically switches the direction of all the Digital I/O lines to input
 *		- The MP35/MP36 has 8 digital input lines (digital lines 0 through 7)
 *		- The MP150 has 16 digital input lines (digital lines 0 through 15)
 *
 *	\param n the Digital Input channel to set  where 0 &lt;= <em>n</em> &lt; number of digital input channels of the MP device
 ...
 *	The linear scaling and offset are then applied to unscaled values read from the MP device internally. These scaled values are what is returned by the getMostRecentSample(), getMPBuffer(), and receiveMPData().
 *
 *	\note
 *		- The usnscaled values are in the units of voltage, the native units of the MP device
 *		- By the default the scaling factor is one and the offset is zero
 *
 ...
 *	\param scaled1 the firs scaled value
 ...
 *
 *	\note
 *		- For the MP35, only analog output 0 is available. This function is used with setAnalogOutputMode(OUTPUTVOLTAGELEVEL) mode
 *		- For the MP150, <em>maxvolt</em> = 10.0 and <em>minvolt</em> = -10.0
 *		- For the MP35, <em>maxvolt</em> = 4.096 and <em>minvolt</em> = 0.0
 *		- For the MP36, constant voltage level output is not supported
 *
 ...
 *		- One of MP35/MP36 Input channels may be redirected to Output Channel 0
 *		- to reset output signal to zero user should issue this command with parameter of OUTPUTGROUND
 *
 *	\param mode specify the output mode of MP35/MP36 device\n
 *	    if <em>mode</em> == <em>OUTPUTVOLTAGELEVEL</em> switch to CONSTANT voltage level output mode (MP35 device ONLY)\n
 ...
 *		- The MP35/MP36 has 8 digital output lines (digital lines 0 through 7)
 ...
 *		- For the MP35/MP36, the external triggering (<em>MPTRIGEXT</em>) can only be triggered via negative edge (<em>posEdge</em>=<em>false</em>)
